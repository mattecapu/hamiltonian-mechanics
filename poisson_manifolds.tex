...
Notice that Equations~\eqref{eq:inf_adj_and_comm} and \eqref{eq:inf_co_adj_and_comm} show explicitly the linearity of $\ad_X$ and $\ad^*_X$ respect to $X$, which indeed are Lie algebra homomorphisms, as we just mentioned before. 
...
In this formalism, the Jacobi identity for the Poisson tensor can be expressed intrinsically as:
\begin{eqalign}
	[\Pi, \Pi] = 0.
\end{eqalign}
...
\label{lemma:poisson_submanifold}
	...
	\end{eqalign}
	In particular, if this is the case then the Poisson substructure on $N$ is unique as such.
\end{lemma}
\begin{proof}
...
\begin{example}[Euler's equations for a rigid body]
	...
	\end{eqalign}
	where $L=I(\omega)$ is  the angular momentum, $M$ embodies the external forces, and $I$ is the inertial tensor
	\begin{eqalign}
...
\begin{example}
	For the trivial Poisson tensor $\Pi = 0$, $H^r_\Pi \iso \Lambda^r(M)$. Notice that the Poisson cohomology can be infinite dimensional. This is not the case of the de Rham cohomology, which under loose (satisfied) assumptions is finite dimensional. 
\end{example}
