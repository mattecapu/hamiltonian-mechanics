...
\label{const:compl_integr_Ham_system}
	If we introduce the independence of $h_1, \ldots, h_n$ in the previous construction, then the associated vector fields $X_{h_1}, \dots, X_{h_n}$ are independent and the distribution $\mathcal D := \langle X_{h_1}, \dots, X_{h_n} \rangle$ is smooth. Equation eq.~\eqref{eq:Lie_alg_struct_Ham_fields} tells us that 
	\begin{eqalign}
...
\begin{remark}
	From Construction \ref{const:compl_integr_Ham_system} we see that each completely integrable Hamiltonian system can be foliated into Lagrangian submanifolds. 
\end{remark}

\begin{remark}
	If $\dim M = 2$ every Hamiltonian system is completely integrable.
\end{remark}
