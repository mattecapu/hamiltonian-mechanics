...
\begin{proof}
	...
	\begin{enumerate}
		...
		\end{eqalign}
		To get Darboux coordinates we want to eliminate the second term. Let's use the freedom of starting points to make the $V^{ij}$ vanish at $p$. Now, since $t^1, \ldots, t^n$ commute, $\Lie{\partial_{t^i}}dt^j = 0$,  and likewise $\Lie{\partial_{t^i}}dh_j = 0$.\footnote{Recall Equation eq.~\eqref{eq:Lie_deriv_k_form} and $\partial_{t^i} = X_{h_i}$ to compute explicitly these Lie derivatives.} Also $\Lie{\partial_{t^i}} \omega = 0$ by Theorem~\ref{th:flow_of_symp_field}. Hence, for each $k=1,\ldots, n$,
		\begin{eqalign}
...
\begin{example}[$2$-species ``predator--prey'' Lotka--Volterra'' system]
	...
	\begin{figure}[H]
		...
		\end{tikzpicture}
		\caption{Special case $m=0$. Notice that this plot has some problems. Actually the whole $P$-axis correspond to the level set given by $p=0$. In order to obtain the level sets shown in the picture one has to move slightly from the $P$-axis, in order to have a non-zero number of prey. The blue point in the $P$-axis corresponds to the value $\frac{r}{c}$.}
