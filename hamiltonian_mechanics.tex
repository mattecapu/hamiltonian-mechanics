...
\label{ex:sphere_with_height_ham}
	...
	\end{eqalign}
	by antisymmetry, so we just go for the mixed one:
	\begin{eqalign}
...
\label{const:symp_struct_for_ham_mechanics}
	...
	\begin{eqalign}
		\underbrace{\begin{pmatrix} 0 & -1\\1 & 0\end{pmatrix}}_{\omega^\#}\underbrace{\der{}{t} \begin{pmatrix} x \\ m\dot x \end{pmatrix}}_{X_h}
		=
		\underbrace{\begin{pmatrix}\partial_x h \\ \partial_{m\dot x} h\end{pmatrix}}_{dh}
	\end{eqalign}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\end{eqalign}
		\item In the third line, we add and subtract the identical terms marked in blue.
	\end{enumerate}
...
\begin{example}[Spherical pendulum]
	...
	Let's check the points where $dh_1$ is not linearly independent from $dh_2$ (as with the singular points check for $dh$, this is in order to satisfy the hypothesis of Arnold--Liouville). As $dh_2 = d\xi$, this happens only when $dh_1$ has just the $d\xi$ term, i.e. the other two vanish:
	\begin{eqalign}
...
\begin{example}[Kepler's problem on the plane]
	...
	\begin{eqalign}
		...
		dh_2 &= dp_\theta
	\end{eqalign}
	thus they're independent unless
	\begin{eqalign}
