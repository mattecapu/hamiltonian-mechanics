...
\begin{definition}
	Let $M$ be a symplectic manifold. A \textbf{Lagrangian fibration} is a submersion\footnotemark $H : M \to N$ where all of the fibres are Lagrangian submanifolds.
\end{definition}
\footnotetext{A submersion $H : M \to N$ is a smooth function such that $dH$ is pointwise surjective.}

\begin{remark}
	Let $H : M \to \R^n$ be the application defined in Construction \ref{const:level_submanifolds_of_involutions}, then it is a Lagrangian fibration if it has no singular points, i.e. if $dh_1\vert_p, \ldots dh_n \vert_p$ are linearly independent at each $p \in M$.
\end{remark}

\begin{remark}
\label{rem:submersion_to_Lag_fibration} 
	Let $H : M \to N$ be a submersion, then the set of critical points are given by the vanishing of $\det dH$, hence it is closed. Then whenever $M$ is symplectic and fibres of $H$ are Lagrangian submanifolds of $M$, we can promote $H$ to a Lagrangian fibration by restricting $M$ to the open set of regular points of $H$.
\end{remark}
...
\label{th:arnold_liouville}
	Let $(M, \omega)$ be a $2n$-dimensional symplectic manifold equipped with $h_1, \ldots, h_n \in \Cinfty(M)$ functions in involution and define $H = (h_1, \ldots, h_n): M \to \R^n$. Suppose $H$ is a Lagrangian fibration.\footnote{We can always restrict to this case as described in Remark \ref{rem:submersion_to_Lag_fibration}.} Let $p \in M$ such that the flows of $X_{h_1}, \ldots, X_{h_n}$, starting at $p$, are complete\footnotemark, and call $F^{(p)}$ the connected component of $H^{-1}(H(p))$ containing $p$. Then
	\begin{enumerate}
