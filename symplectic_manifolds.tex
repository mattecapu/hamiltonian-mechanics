...
\section{Symplectic forms on vector spaces}
\subsection{Skew-symmetric bilinear maps}
\begin{definition}
	Let $V$ be an $\R$-vector space with $\dim V=m$. Let $\Omega:V\times V\to\R$ be a bilinear map. We say $\Omega$ is \textbf{skew-symmetric} iff 
	$$
		\Omega(v,w)=-\Omega(w,v),\qquad\forall v,w\in V.
	$$
\end{definition}
...
\subsection{Symplectic vector spaces}
	Let $V$ be an $n$-dimensional $\R$-vector space. Let $\Omega:V\times V\to \R$ be a bilinear map. There is an induced linear map $\tilde\Omega:V\to V^*$ defined by 
	\begin{eqalign}
		\tilde\Omega(v)(w):=\Omega(v,w).
	\end{eqalign} In other words, we can say that $\Omega$ is an element of $V^*\tens V^*\iso\Hom(V\tens V,\R)\iso\text{Hom}(V,V^*)$. We have 
\begin{eqalign}\tens
...
\begin{remark} 
	The above theorem actually describes the Jordan decomposition for the map $\tilde\Omega:V \to V^*$. Obviously, if we considered the complex field this map could be diagonalized.
	In addition, the definition of symplectic form requires that the Jordan decomposition of $\tilde\Omega$ has no zero-eigenvalue blocks.
\end{remark}
...
Equation~\eqref{eq:can_form} prescribes we can always find a ``nice'' basis for $V$ in which $\Omega$ can be represented by a \textbf{symplectic matrix}:
\begin{eqalign}
	\begin{pmatrix}
		0 & I_n\\
		-I_n & 0
...
\begin{example}
	Let $(V,\Omega)$ be a symplectic vector space with $V=\langle e_1,\dots,e_n,f_1,\dots,f_n\rangle$ where $e_1,\dots,e_n,f_1,\dots,f_n$ is a canonical basis. Then
	\begin{enumerate}
...
This is equivalent to requiring that the following diagram commutes:
\tikzset{    labl/.style={anchor=south, rotate=90, inner sep=-2mm}}
\begin{diagram}
		V\arrow[shift left=1em, "\textasciitilde" labl]{d}{\phi}\times V\arrow[shift right=1em,"\textasciitilde" labl]{d}{\phi}\arrow{r}{\Omega} \& \R\arrow["\textasciitilde" labl]{d}{}\\
		V'\times V'\arrow{r}{\Omega}\& \R
\end{diagram}

\begin{remark}
	Notice that an homomorphism $\phi:V\overset\sim\longmapsto V' $ between symplectic spaces is forced to be an isomorphism by the compatibility condition with the symplectic forms.
\end{remark}
...
Notice that for vector spaces equipped with a non-degenerate \emph{symmetric} form (\textbf{inner-product spaces}) the situation is different, since on $\R^n$ we can define at least $n$ different metrics differing by signature, i.e. any real inner-product space $(V,g)$ is isomorphic to a ``canonical'' space $(\R^n,g_0)$ of the form
\begin{eqalign}
...
\begin{definition}
	Let $M$ be a smooth, $m$-dimensional manifold. The 2-form $\omega\in\Omega^2(M)$ is a \textbf{symplectic form}  if $d \omega=0$ (it's closed) and $\omega\vert_p$ is symplectic (on $T^*M$) $\forall p\in M$. A \textbf{symplectic manifold} is a pair $(M,\omega)$ of a smooth manifold $M$ and a symplectic form $\omega$ on it.
\end{definition}

Therefore at each point $p \in M$ the tangent space $T_pM$ is a symplectic vector space with symplectic form $\omega_p$.

\begin{remark} 
	If $\omega\in\Omega^2(M)$ is symplectic then $\dim M=2n$.
\end{remark}
...
\label{ex:eu_sp_sphere}
	...
	\begin{enumerate}
		...
		\end{eqalign}
		where $x_1,\dots,x_n,y_1,\dots,y_n$ are linear coordinates on $\R^{2n}$. With this definition $\omega_0$ is, pointwise, the form $\Omega_0$ discussed previously. Moreover, it is closed as
		\begin{eqalign}
			d\omega_0 &= \sum_{i=1}^n d(dx^i \wedge dy^i)\\
			&= \sum_{i=1}^n d^2x^i \wedge y^i - dx^i \wedge d^2y^i = 0
		\end{eqalign}
		\item The $2$-sphere is a symplectic manifold. Indeed, consider its immersion in $\R^3$ defined as $S^2 = \{ p \in \R^3 \suchthat \|p\| = \langle p,p\rangle = 1\}$. Then
		\begin{eqalign}
			T_pS^2 = \{ v \in \R^3 \suchthat \langle v, p \rangle = 0 \}=\langle p\rangle^\perp
		\end{eqalign}
		can be made into a symplectic vector space by means of
		\begin{eqalign}
			\omega\vert_p (u,v) := \langle p, u \times v \rangle, \quad \forall u,v \in T_pS^2.
		\end{eqalign}
		which trivially defines a differential form $\omega \in \Omega^2(S^2)$. $\omega$ is nondegenerate because $\langle p,u\times v\rangle\neq0$ when $u\neq0$ and, for instance, $v=u\times p$. Finally, since $\dim S^2 = 2$, the form $\omega$ is necessarily closed.
	\end{enumerate}
\end{example}

Since $\dim T_pM = \dim M$, $M$ inherits the condition on the dimension of $T_pM$: $M$ must be even-dimensional. However this time, even-dimensionality is not a sufficient condition for a manifold to admit symplectic forms. A further necessary condition is given by the following proposition.

\begin{proposition}
\label{prop:symp_vol_form}
	Let $(M, \omega)$ be a symplectic manifold, with $\dim M = 2n$. Then the $n$-th exterior power of $\omega$ is a volume form, i.e. $\omega^{\wedge n}\not\equiv 0$ everywhere on $M$.
\end{proposition}
\begin{proof}
	The rank of $\omega^{\wedge n}\in\Omega^{2n}(M)$ is $2n$, so the claim makes sense. Choose $p \in M$ now, and fix a local frame $E_1, \ldots, E_n, F_1, \ldots, F_n$ around $p$ in such a way that  it is in $p$ a canonical basis as per Theorem~\ref{th:decomp_th}. Then:
	\begin{eqalign}
...
The obvious remark is then that non-orientable manifolds can't be symplectic even in the even-dimensional case; e.g. Möbius strip and the projective plane do not admit a symplectic structure.
...
\begin{proof}
	...
	\begin{eqalign}
		\int_{S^{2n}} \omega^{\wedge n} = \int_{S^{2n}} d(\eta\wedge\omega^{\wedge (n-1)}) = \int_{\partial S^{2n}} \eta\wedge\omega^{\wedge (n-1)} = 0
	\end{eqalign}
	Then, by Proposition~\ref{prop:symp_vol_form}, it must be $\omega = 0$ thus not symplectic.
\end{proof}
...
\begin{definition} 
	Let $(M_1,\omega_1)$ and $(M_2, \omega_2)$ be two symplectic manifolds of the same dimension. A diffeomorphism $\phi:M_1\to M_2$ is called a \textbf{symplectomorphism} if
	\begin{eqalign}
		\phi^* \omega_2 = \omega_1
	\end{eqalign}
\end{definition}
...
\begin{remark}
	If $N$ is a submanifold of a symplectic manifold $(M,\omega)$ and $(N, \omega\vert_{N})$ is symplectic then $N$ is a \textbf{symplectic submanifold} of $M$. In particular a symplectomorphism $\phi:M\to N$ is an embedding.
\end{remark}
